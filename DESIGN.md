...
```css
/* === Brand (Primary) === */
--color-primary:         #00a728;  /* MOS Green: CTA・強調 */
--color-primary-dark:    #006436;  /* MOS Green Dark: 強調見出し・グリーン面上テキスト */
--color-primary-deeper:  #004924;  /* MOS Green Deeper: ダークセクション */
--color-primary-darkest: #002614;  /* MOS Green Darkest: フッター背景等 */
--color-primary-pale:    #d9f2df;  /* MOS Green Pale: 薄いグリーン背景・アクセント面 */
--color-secondary:       #ff7e47;  /* MOS Orange: サブアクション */

/* === Danger（エラー表示専用。ブランドアクセントからは分離） === */
--ncom-color-red:        #e60012;
...
- **MOS Green** (`#00a728`): CTA ボタン・主要アクション・強調点に使用
...
- **MOS Orange** (`#ff7e47`): サブアクション用のセカンダリカラー

### Danger（エラー表示用）

- **Red** (`#e60012`) / **Deep Red** (`#cf0001`): フォームのエラーメッセージ等、危険・失敗状態の表示専用。ブランドアクセントとしては使用しない
...
```
# Brand
MOS Green:         #00A728   ← CTA・強調
MOS Green Dark:    #006436   ← 強調見出し・グリーン面上テキスト
MOS Green Deeper:  #004924   ← ダークセクション
MOS Green Darkest: #002614   ← フッター背景等
MOS Green Pale:    #D9F2DF   ← 薄いグリーン背景・アクセント面
MOS Orange:        #FF7E47   ← サブアクション
...
- 主要 CTA: bg #00A728 / color: #FFFFFF / hover bg: #006436
